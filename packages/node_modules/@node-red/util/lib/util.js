...
function evaluateNodeProperty(value, type, node, msg, callback) {
    ...
    if (type === 'str') {
      ...
      var globalGet = function (key,cb) {
        return node.context().global.get(key,value,cb);
      }
      var globalSet = function (key,val) {
        return node.context().global.set(key,val);
      }
      var flowGet = function (key,cb) {
        return node.context().flow.get(key,value,cb);
      }
      var flowSet = function (key,val) {
        return node.context().flow.set(key,val);
      }
      var result = {
        global: {
          get: globalGet,
